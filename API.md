...
- 服务端对所有接口启用**全局 IP 级别速率限制**（默认 600 请求/分钟）
...
用于兑换码、友链申请等高风险写操作的人机验证。

### 获取 Cap 配置

```http
GET /api/v1/cap/config
```
...
> 注意：该接口返回**原始 JSON**（不是 `{code,message,data}` 包装）。
...
> **多角色支持**：一个鹰角账号下可能绑定多个终末地游戏角色（如官服、B站服）。登录成功后可通过手机/Cred 登录响应，或扫码确认接口 `POST /login/endfield/qr/confirm` 获取 `available_roles` 字段。
>
> **典型使用流程**：
> 1. 调用登录接口获取 `framework_token`
> 2. 获取可用角色列表：扫码登录使用 `POST /login/endfield/qr/confirm`；手机/Cred 登录会在响应中直接返回 `available_roles`
> 3. 展示角色列表供用户选择
> 4. 对每个要绑定的角色调用 `POST /api/v1/bindings`，传入 `role_id` 等信息
> 5. 查询数据时，便捷端点支持 `?roleId=xxx` 指定角色；不传则使用主绑定角色
...
| msg | string | 状态消息 |
| status | string | 状态标识 |
| expire | int64 | 过期时间戳（毫秒，`pending/scanned` 时返回） |
| remaining_ms | int64 | 剩余有效时间（毫秒，`pending/scanned` 时返回） |
...
> `available_roles` 由确认登录接口 `POST /login/endfield/qr/confirm` 返回。
...

{
  "phone": "13800138000",
  "code": "123456",
  "framework_token": "可选，来自 /phone/send"
}
```
...
| 参数 | 说明 |
|------|------|
| X-User-Identifier | 无 JWT 时可通过该 Header 传递用户标识 |
| X-Client-Type | 可选客户端类型：web/bot/third_party |
| X-Client-ID | 可选客户端标识 |
...
DELETE /api/v1/bindings/:id
X-API-Key: your-api-key
X-User-Identifier: QQ12345
```

> 注意：
> - 第三方客户端除 `X-API-Key` 外，还必须提供用户标识：`X-User-Identifier`（或 `?user_identifier=xxx`）
> - 只能删除自己创建的绑定（按 API Key 所有者隔离）
...
> 下文示例主要展示 `X-Framework-Token`，实际调用时还需同时携带一项接口认证凭证（API Key / JWT / Anonymous Token）。
...
```http
POST /api/blueprints/callback/qiniu-audit
```

用于接收七牛云图片增量审核回调，更新图片审核状态（`approved` / `rejected`）。

**说明**：
- 该接口不走 JWT / API Key 认证，依赖**七牛云回调签名验证**。
- 回调签名无效时返回 `401`。
- 审核结果会写入 `blueprint_image_audits`，并异步更新已引用该图片的蓝图状态。
...
>
> ⚠️ 当前实现未内置 `X-Admin-Secret` 路由鉴权，请务必通过网关、WAF 或内网访问策略限制这些接口暴露范围。

### 接口概览

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/endfield/admin/check-credentials` | 手动触发有效凭证检查任务（异步） |
| POST | `/api/endfield/admin/cleanup-credentials` | 手动触发失效凭证清理任务（异步） |
| GET | `/api/endfield/admin/credential-status` | 获取凭证统计和任务配置 |

### 凭证自动清理机制

系统内置凭证管理插件，包含“有效性检查 + 失效清理”两类任务：

| 配置项 | 值 | 说明 |
|--------|-----|------|
| 检查间隔 | 30 分钟 | 定时验证 `is_valid=true` 的凭证可用性（失败会标记失效） |
| 清理间隔 | 6 小时 | 定时清理失效凭证 |
| 保留天数 | 7 天 | 失效后保留 7 天再删除 |
| 启动检查 | 15 秒后 | 服务启动后自动执行一次检查任务 |
...
> 注意：上述手动接口均在后台异步执行，返回成功不表示任务已执行完毕。
...
> ⚠️ 需要认证：`X-API-Key: <your-api-key>`。推荐优先使用 `status` 接口。

```http
GET /api/v1/authorization/requests/:request_id/data
X-API-Key: your-api-key
```
...
```json
{
  ...
  "data": {
    ...
    "subscription": {
      ...
      "daily_used": 1234,
      "daily_remaining": 48766,
      "monthly_used": 45000,
      "monthly_remaining": 955000
    },
    "api_key_usage": [
      {
...
```http
GET /api/v1/proxy-users/avatar/:id
```

公开头像代理接口，按代理用户 ID 返回头像内容（或 302 跳转到存储头像）。

**说明**：
- 无需认证，可直接用于前端展示头像。
- 当代理用户设置了 `avatar_source`（如 QQ 头像）时，服务端会反向代理并隐藏上游敏感信息。
- 返回 `404` 表示用户不存在或未设置头像。
...
> ⚠️ 订阅相关认证规则：
> - 公开接口（无需 JWT）：`/api/v1/subscription/plans`、`/api/v1/subscription/epay/notify`、`/api/v1/subscription/epay/return`、`/api/v1/subscription/captcha-config`
> - 管理端接口（Admin Secret）：`/api/v1/subscription/redemption/create`、`/api/v1/subscription/redemption/list`、`/api/v1/subscription/redemption/disable`
> - 其余用户接口需要 Web JWT：`Authorization: Bearer <access_token>`
>
...
> 以下数值以仓库默认 `config/subscription.yaml` 为准；生产环境可能不同，请以 `GET /api/v1/subscription/plans` 实时返回为准。

| 计划 | 名称 | 访问级别 | 速率限制 | 日配额 | 月配额 | 月付价格 | 年付价格 |
|------|------|----------|----------|--------|--------|----------|----------|
| free | Free | public | 300 次/分钟 | 10,000 | 30,000 | 免费 | 免费 |
| pro | Pro | standard | 600 次/分钟 | 50,000 | 1,000,000 | ¥9.00 | ¥90.00 |
| plus | Plus | premium | 无限制 | 无限制 | 无限制 | ¥15.00 | ¥150.00 |
...
| 类型 | 名称 | 请求量 | 价格 |
|------|------|--------|------|
| pack_5000 | 5000次请求包 | 5,000 | ¥5.00 |
| pack_20000 | 20000次请求包 | 20,000 | ¥20.00 |
| pack_50000 | 50000次请求包 | 50,000 | ¥50.00 |
...
```json
{
  ...
  "data": {
    ...
    "quota_packs": [
      { "type": "pack_5000", "name": "5000次请求包", "amount": 5000, "price": 5.00 },
      { "type": "pack_20000", "name": "20000次请求包", "amount": 20000, "price": 20.00 },
      { "type": "pack_50000", "name": "50000次请求包", "amount": 50000, "price": 50.00 }
    ]
  }
}
```
...
```http
GET /api/v1/subscription/captcha-config
```

返回前端初始化阿里云验证码所需参数（启用时返回 `scene_id` 和 `prefix`）。
...
#### 管理端兑换码接口（Admin Secret）

以下接口用于运营后台，需请求头 `X-Admin-Secret`：

- `POST /api/v1/subscription/redemption/create` 创建兑换码（支持批量）
- `GET /api/v1/subscription/redemption/list` 分页查询兑换码
- `POST /api/v1/subscription/redemption/disable` 禁用兑换码
...
| 方式 | 权限级别 | 有效期 | 说明 |
|------|----------|--------|------|
| **月付/年付订阅** | 对应计划级别 | 按周期 | Pro (¥9.00/月) / Plus (¥15.00/月) |
...
> 用于 MaaEnd 设备绑定、任务下发、任务查询与实时推送。
>
> **认证方式（HTTP API）**：`Authorization: Bearer <access_token>` 或 `X-API-Key`
>
> **WebSocket 端点**：
> - 设备侧：`GET /ws/maaend`（连接后在协议层完成认证）
> - 用户侧：`GET /api/maaend/ws`（需要 JWT 或 API Key）

### 接口概览

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/maaend/devices/bind-code` | 生成设备绑定码（5 分钟有效） |
| GET | `/api/maaend/devices` | 获取当前用户的设备列表 |
| PATCH | `/api/maaend/devices/:device_id` | 修改设备名称等信息 |
| DELETE | `/api/maaend/devices/:device_id` | 删除设备 |
| GET | `/api/maaend/devices/:device_id/tasks` | 获取设备可用任务列表 |
| POST | `/api/maaend/devices/:device_id/tasks` | 提交任务执行请求 |
| POST | `/api/maaend/devices/:device_id/reset` | 重置设备任务状态（恢复卡住设备） |
| GET | `/api/maaend/devices/:device_id/screenshot` | 获取设备截图 |
| GET | `/api/maaend/jobs` | 获取任务历史 |
| GET | `/api/maaend/jobs/:job_id` | 查询任务状态 |
| POST | `/api/maaend/jobs/:job_id/stop` | 停止任务 |
| GET | `/api/maaend/ws` | 用户实时推送 WebSocket（需认证） |
| GET | `/ws/maaend` | 设备连接 WebSocket |
...
# 4. 获取匿名访问凭证（也可改用 JWT 或 API Key）
ANON_TOKEN=$(curl -s -X POST http://localhost:15618/api/v1/auth/anonymous-token \
  -H "Content-Type: application/json" \
  -d '{"fingerprint":"your-browser-fingerprint-at-least-32-chars"}' \
  | jq -r '.data.token')

# 5. 调用数据 API（接口认证 + 游戏凭证）
curl -H "X-Anonymous-Token: $ANON_TOKEN" \
  -H "X-Framework-Token: xxx" \
  http://localhost:15618/api/endfield/user

# 6. 签到
curl -X POST \
  -H "X-Anonymous-Token: $ANON_TOKEN" \
  -H "X-Framework-Token: xxx" \
...
- ✅ **数据授权接口安全增强**
  ...
  - 轮询授权状态 (`GET /api/v1/authorization/requests/:request_id/status`) 需要同一 API Key
  - 获取授权数据 (`GET /api/v1/authorization/requests/:request_id/data`) 需要同一 API Key
...
- ✅ **凭证清理插件（取代刷新插件）**
  - 每 30 分钟自动检查有效凭证可用性（失败自动标记失效）
  - 每 6 小时自动清理失效凭证（`is_valid=false` 且超过 7 天）
  - 清理日志包含详细统计

- ✅ **管理接口变更**
  - `POST /api/endfield/admin/check-credentials` - 手动触发凭证检查
